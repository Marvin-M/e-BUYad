...
class CodeController extends BaseController
{
    ...
    private function getMCCode($id){
        $lastCode = DB::table('tblMemCardDeactivated')
                        ->select('strMCardId')
                        ->where('strMCardCode','=',$id)
                        ->orderBy('strMCardId','desc')
                        ->take(1)
                        ->get();
            return $lastCode;
    }

    public function getLastMemCardCode($id)
    {
        $strCode = $this->getMCCode($id);
        if($strCode == null || $strCode == "") {
            $forCode = "FCK00000";
        }
        else {
            $forCode = "FCK".$strCode["0"]->strMCardId;
        }

        return substr($this->showCode($forCode),3,5);
    }
}
