  ...
  <div class="panel-body">
    <div class="content-row">
      ...
      <div class="panel panel-default">
        <div class = "row">
          <div class = "col-md-3">
            <button type="button" class="btn btn-primary btn-block" href="#pkg_form" data-toggle="collapse" onclick="clearForm()">ADD PACKAGE</button>
          </div>
        </div>
        ...
        <div class ="row">
          <div class="col-md-offset-1 col-md-10">
            <div id="pkg_form" class="collapse">
              <form id="packageform" class="form-horizontal" role="form" method="post" action="">
                ...
                <div class="form-group"><!-- Validity Start -->
                ...
                <div class="form-group"><!-- Validity End -->
                ...
                <div class="col-md-12"><!-- Select Products -->
                  <div class="col-md-6">
                    <div class="form-group">
                      <label class="control-label col-md-6">Select Products</label>
                    </div>
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="table-responsive"><!-- Selected Products Table -->
                ...
                <div class="form-group"><!-- Package Price -->
                ...
                <div class="form-group"><!-- Form Buttons -->
                  <div class="col-md-offset-3 col-md-10">
                    ...
                    <button class="btn btn-info" type="cancel" href="#pkg_form" data-toggle="collapse">Cancel</button>
                  </div>
                </div>
                            ...
                            <!-- php script here-->
                                                    ...
                                                    $results = DB::select("SELECT 
                                                                ...
                                                                foreach($results as $data){
                                                                    ...
                                                                    echo '<td><a href="#View" class="" data-toggle="modal" data-target="#View" onclick="getProducts(\''.$data->strPackCode.'\');">View Details</a></td>';
                                                                    ...
                                                                    echo '
                                                                        ...
                                                                        <td align="center">
                                                                          <table>
                                                                            <tr>
                                                                                <button type="button" class="btn btn-success btn-block" href="#pkg_form" data-toggle="collapse" onClick="setFormData(\''.
                                                                                                                                                                                                        $data->strPackCode.'\',\''.
                                                                                                                                                                                                        $name.'\',\''.
                                                                                                                                                                                                        $data->datPackFrom.'\',\''.
                                                                                                                                                                                                        $data->datPackTo.'\',\''.
...
<script type="text/javascript">
  ...
  function qtyedit(x){
    x.innerHTML = "<input onkeypress='heyhehehe(event, this)' id='edittxt' type='number' value='" + x.innerHTML + "'>";
    document.getElementById('edittxt').focus();
  }

  function heyhehehe(e, x){
    if(e.keyCode == 13){
...
<script>
    function getProducts(pcode){
      $("#partprod tbody").html("");
      $.ajax({
            url: '/maintenance/ppd/packages/get-participating-products',
            type: 'GET',
            data: {
                packcode: pcode
            },
            success: function(data){
              ...
              for(var i = 0; i < data.length; i++){
                ...
                if(data[i]['strProdType'] == 0){
                  if(data[i]['strPMBranName'] != null){
                    trows = trows + "<b>" + data[i]['strPMBranName'] + "</b> ";
                  }
                  trows = trows + data[i]['MedName'];
                }else{
                  trows = trows + data[i]['NMedName'];
                }
                ...
                //price
                trows = trows + "<td>";
                trows = trows + data[i]['decProdPricePerPiece'];
                trows = trows + "</td>";

                //quantity
                trows = trows + "<td>";
                trows = trows + data[i]['intPackProdQuantity'];
                trows = trows + "</td>";

                trows = trows + "</tr>";
              }

              $("#partprod tbody").html(trows);

            }, 
            error: function(){
              alert('Products in the package cannot be shown');
            }
        });
    }
</script>
...
<script type="text/javascript">
  function setFormData(code,name,start,end,tpr,ppr){
    name = name.replace("#","'");
    $("#packcode").val(code);
    $("#name").val(name);
    $("#start").val(start);
    ...
    $("#part").DataTable().rows().remove().draw();

    document.getElementById("packageform").action="{{URL::to('/maintenance/ppd/packages/update-package')}}";
    ...
    $.ajax({
          ...
          success: function(data){
            for(var i = 0; i < data.length; i++){
              ...
              if(data[i]['strProdType'] == 0){
                if(data[i]['strPMBranName'] != null){
                  prodname = prodname + data[i]['strPMBranName'];
                }
                prodname = prodname + "(" + data[i]['MedName'] + ")";
              }else{
                prodname = prodname + "(" + data[i]['NMedName'] + ")";
              }
              ...
              $('#part').DataTable().row.add( [
                  ...
                  prodname,
                  data[i]['decProdPricePerPiece'],
                  data[i]['intPackProdQuantity'],
                  data[i]['strProdCode']
              ] ).draw();

              document.getElementById('packprods').value = document.getElementById('packprods').value + data[i]['strProdCode'] + ";";
              var table = document.getElementById("part");
              var row = table.rows[table.rows.length-1];
              var cell = row.cells[3];
              ...
              cell.outerHTML = '<td>' + 
                               '<button onclick="removeRow(this,\''+data[i]['strProdCode']+'\');"; class="btn btn-danger btn-block" type="button"  data-toggle="modal" data-target="#myConfirm"><i class="glyphicon glyphicon-remove"></i></button>' +
                               '</td>';
            }
          }, 
          error: function(){
  ...
  function clearForm(){
    ...
    document.getElementById("totalpr").value = "0";
    document.getElementById("pkgprice").value = null;
    $("#part").DataTable().rows().remove().draw();
    document.getElementById("packageform").action="{{URL::to('/maintenance/ppd/packages/add-package')}}";
  }
</script>
@stop